...
//   alert("회원가입을 해주세요.", G5_BBS_URL."/login.php?url=".urlencode(G5_URL."/check_list.php"));
// }
if ($member['mb_level'] <= 5){
  alert("점검중 입니다.", G5_BBS_URL."/login.php?url=".urlencode(G5_URL."/check_list.php"));
}
                  ...
                  echo '</div>';
                      echo '<div class="card-body">';
                        echo '<div id="relation_keyword" class="row no-gutters align-items-center">';
                          ...
                          echo '<thead><tr style="border-bottom: solid 1px;"><td>번호</td><td>포스팅 제목</td><td>포스팅 시간</td><td>포스팅 글자수</td><td>포스팅 노출</td><td>이미지 노출</td><td>분석 키워드</td><td>키워드 노출</td><td>#태그 분석</td></tr></thead>';
                          echo '<tbody>';
                          $lo_num = 0;
                          $log_arrays = [];
                          $t_arrays = [];
                          ...
                          $blog_array = True;
                          $number = 1;
                          $url = ((string)'https://blog.naver.com/PostTitleListAsync.nhn?blogId='.($keword).'&viewdate=&currentPage='.(string)$number.'&categoryNo=&parentCategoryNo=&countPerPage=20');
                          ...
                          echo '<button id="count_btn" onclick="tableCreate()">table_append</button>';
                        echo '</div>';
                      echo '</div>';
                    echo '</div>';
                  echo '</div>';
